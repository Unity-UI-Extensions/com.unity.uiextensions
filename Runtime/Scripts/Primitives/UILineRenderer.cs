...
namespace UnityEngine.UI.Extensions
{
	...
	public class UILineRenderer : UIPrimitiveBase
	{
		...
		[SerializeField, Tooltip("Close the line by joining the last point back to the first\nCompletes the final corner for shapes like boxes")]
		internal bool closedLine;
		...
		/// <summary>
		/// For a closed line, make sure the path returns to its first point so the final edge is drawn. If the caller already repeated the first point we leave it alone to avoid a zero-length segment.
		/// </summary>
		public bool LineClosed
		{
			get { return closedLine; }
			set { closedLine = value; SetAllDirty(); }
		}
		...
		private void PopulateMesh(VertexHelper vh, Vector2[] pointsToDraw)
		{
			...
			if (closedLine && !lineList && pointsToDraw.Length > 2 &&
				pointsToDraw[0] != pointsToDraw[pointsToDraw.Length - 1])
			{
				var closedPoints = new List<Vector2>(pointsToDraw);
				closedPoints.Add(pointsToDraw[0]);
				pointsToDraw = closedPoints.ToArray();
			}
			...
			else
			{
				...
				for (var i = 1; i < pointsToDraw.Length; i++)
				{
					...
					if (lineCaps && !closedLine && i == pointsToDraw.Length - 1)
					{
						segments.Add(CreateLineCap(start, end, SegmentType.End));
					}
				}
			...
			if (closedLine && !lineList && segments.Count > 2)
			{
				DrawSegmentJoin(vh, segments[segments.Count - 1], segments[0]);
			}
			...
			for (var i = 0; i < segments.Count; i++)
			{
				...
				{
					DrawSegmentJoin(vh, segments[i], segments[i + 1]);
				}
		...
		// Creates the join between two consecutive line segments, mitring or bevelling
		// the shared corner. segmentA is the incoming segment, segmentB the outgoing one.
		private void DrawSegmentJoin(VertexHelper vh, UIVertex[] segmentA, UIVertex[] segmentB)
		{
			var vec1 = segmentA[1].position - segmentA[2].position;
			var vec2 = segmentB[2].position - segmentB[1].position;
			...
			var miterPointA = segmentA[2].position - vec1.normalized * miterDistance * sign;
			var miterPointB = segmentA[3].position + vec1.normalized * miterDistance * sign;
			...
			if (joinType == JoinType.Miter)
			{
				...
				{
					segmentA[2].position = miterPointA;
					segmentA[3].position = miterPointB;
					segmentB[0].position = miterPointB;
					segmentB[1].position = miterPointA;
				}
				else
				{
			...
			if (joinType == JoinType.Bevel)
			{
				...
				var join = new UIVertex[] { segmentA[2], segmentA[3], segmentB[0], segmentB[1] };
